...
func get_next_scene_text() -> String:
    ...
    # if we've reached a new scene number and haven't shown it yet
    if scene_number > current_scene and not shown_scenes.has(scene_number):
        current_scene = scene_number
        shown_scenes.append(scene_number)

        match scene_number:
            1: return MissionText.scene_1
    ...
    # If we don't have a new scene to show, and we haven't shown the opening monlogue yet
